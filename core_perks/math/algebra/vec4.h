...
namespace cp
{
	...
	{
	public:
		CP_FORCE_INLINE Vec4() : x_(0), y_(0), z_(0), w_(0) {}
		...
		CP_FORCE_INLINE T& operator[](int idx) { CP_ASSERT(idx < 4); return xyzw_[idx]; }
		CP_FORCE_INLINE T operator[](int idx) const { CP_ASSERT(idx < 4); return xyzw_[idx]; }
